...
export default function AvatarWithBadge({
  ...
  const badgeSizeClasses = {
    ...
    md: "w-6 h-6 -top-1.5 -left-1.5",
    lg: "w-8 h-8 -top-2 -left-2",
    xl: "w-10 h-10 -top-2.5 -left-2.5",
  };
  ...
  return (
    ...
    >
      ...
      <Link href="/achievements" title="Xem danh hiệu">
        ...
        >
          <div className={`relative w-full h-full rounded-full border-[1.5px] border-white shadow-[0_2px_5px_rgba(0,0,0,0.15)] bg-[#Fdfbf7] overflow-hidden ${isLocked ? 'grayscale opacity-70' : ''}`}>
            {/* Using a placeholder for now, ideally next/image */}
            <img src={badgeImg} alt={title} className="w-full h-full object-cover" />
